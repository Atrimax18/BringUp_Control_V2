...
namespace BringUp_Control
{
    internal class FPGA : IDisposable
    {
        ...
        string coinfile = string.Empty;
        public string Coindigitaldatafile
        {
            get { return coinfile; }
            set { coinfile = value; }
        }
        ...
        private FPGARegisterLoader regLoader = new FPGARegisterLoader();

        private const int BC = 12; // Bit count for I/Q components
        private const int MaxRetries = 3;

        public static Dictionary<string, uint> RegisterMapTOP_Container = new Dictionary<string, uint>();
        ...
        public void LoadRegisterFile(string filePath)
        {
            ...
            try 
            {
                ...
                RegisterMapTOP_Container = regLoader.LoadRegisterMapFromHtml(coinfile);
                if (RegisterMapTOP_Container.Count == 0)
                {
                    MessageBox.Show("Failed to load register map from HTML file.");
                    return;
                }
                else
                {
                    MainForm.Instance.LogStatus("Register map loaded successfully with " + RegisterMapTOP_Container.Count + " entries.");
                }
            }
            catch (Exception ex)
            {
